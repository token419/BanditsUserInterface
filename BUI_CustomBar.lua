BUI.CustomBar = {}
BUI.CustomBar.BarContent = {}
BUI.CustomBar.LeaderCommands = {}
BUI.CustomBar.LeaderCommands[1] = {enable=false, mess="Come to the crown", text="", icon="/esoui/art/icons/ability_warrior_021.dds"}
BUI.CustomBar.LeaderCommands[2] = {enable=false, mess="Start fight", text="", icon="/esoui/art/icons/ability_warrior_018.dds"}
BUI.CustomBar.LeaderCommands[3] = {enable=false, mess="Wipe", text="", icon="/esoui/art/icons/ability_mage_019.dds"}
BUI.CustomBar.LeaderCommands[4] = {enable=false, mess="Break 2 min", text="2", icon="/esoui/art/icons/ability_mage_044.dds"}
BUI.CustomBar.LeaderCommands[5] = {enable=false, mess="Break 5 min", text="5", icon="/esoui/art/icons/ability_mage_044.dds"}
BUI.CustomBar.LeaderCommands[6] = {enable=false, mess="Break 10 min", text="10", icon="/esoui/art/icons/ability_mage_044.dds"}
-- BUI.CustomBar.LeaderCommands[7] = {enable=false, mess="Place marker 1", text="1", icon="/esoui/art/icons/housing_gen_inc_soulgemdoormarkerssmall002.dds"}
-- BUI.CustomBar.LeaderCommands[8] = {enable=false, mess="Place marker 2", text="2", icon="/esoui/art/icons/housing_gen_inc_soulgemdoormarkerssmall002.dds"}

BUI.CustomBar.SlashCommands = {}
	--ReloadUI
BUI.CustomBar.SlashCommands[1] = {enable=true, command="/reloadui", icon="/esoui/art/mounts/ridingskill_ready.dds"}
	--Recruit
BUI.CustomBar.SlashCommands[2] = {enable=false, command="/script StartChatInput('/z Guild [name] recruits new members!')", icon="/esoui/art/icons/ability_warrior_010.dds"}
	--Dance
BUI.CustomBar.SlashCommands[3] = {enable=false, command="/dancedunmer", icon="/esoui/art/icons/ability_mage_066.dds"}
	--CompassFrame
BUI.CustomBar.SlashCommands[4] = {enable=true, command="/script ZO_CompassFrame:SetHidden(not ZO_CompassFrame:IsHidden())", icon="/esoui/art/icons/ability_rogue_062.dds"}
	--Mimewall
BUI.CustomBar.SlashCommands[5] = {enable=false, command="/mimewall", icon="/esoui/art/icons/emote_mimewall.dds"}
	--GemStone
BUI.CustomBar.SlashCommands[6] = {enable=true, command="/script UseCollectible(336)",icon="/esoui/art/icons/quest_gemstone_tear_0002.dds"}
	--Jumptoleader
BUI.CustomBar.SlashCommands[7] = {enable=false, command="/jumptoleader", icon="/esoui/art/tutorial/gamepad/gp_playermenu_icon_store.dds"}
	--Whisper target
BUI.CustomBar.SlashCommands[8] = {enable=false, command="/script zo_callLater(function() local name=GetUnitDisplayName('reticleover') if name then StartChatInput('/w '..name..' ') else a('No target') end end,100)", icon="esoui/art/tutorial/chat-notifications_up.dds"}
	--Stolen items
BUI.CustomBar.SlashCommands[9] = {enable=false, command="/script d(AreAnyItemsStolen(BAG_BACKPACK) and 'Have stolen items' or 'Have no stolen items')", icon="/esoui/art/inventory/gamepad/gp_inventory_icon_stolenitem.dds"}
	--Purge
BUI.CustomBar.SlashCommands[10] = {enable=false, command="/script local _,i=GetAbilityProgressionXPInfoFromAbilityId(40232) local _,m,r=GetAbilityProgressionInfo(i) local _,_,index=GetAbilityProgressionAbilityInfo(i,m,r) CallSecureProtected('SelectSlotAbility', index, 3)", icon="/esoui/art/icons/ability_ava_005_a.dds"}
	--Widgets
BUI.CustomBar.SlashCommands[11] = {enable=false, command="/script BUI.Vars.EnableWidgets=not BUI.Vars.EnableWidgets BUI.Frames.Widgets_Init() d('Widgets are now '..(BUI.Vars.EnableWidgets and '|c33EE33enabled|r' or '|EE3333disabled|r'))", icon="/esoui/art/progression/morph_up.dds"}
	--Text sample
BUI.CustomBar.SlashCommands[12] = {enable=false, command="/script local text='Another sample'd(text) a(text)", icon="Text"}
...
BUI.CustomBar.UltimateSlot = ZO_ActionBar_GetButton(8)

do	--Default
	local t={Enable=false,Leader={},Slash={}}
	for i,data in pairs(BUI.CustomBar.LeaderCommands) do t.Leader[i]=data.enable end
	for i,data in pairs(BUI.CustomBar.SlashCommands) do t.Slash[i]=data end
...
local function UpdateContent()
	...
	if BUI.Player.isLeader or BUI.inMenu then
		for i,data in pairs(BUI.CustomBar.LeaderCommands) do
			if BUI.Vars.CustomBar.Leader[i] then table.insert(BUI.CustomBar.BarContent,{leader=true,command=i,text=data.text,icon=data.icon}) end
		end
	end
	for i in pairs(BUI.CustomBar.SlashCommands) do
		if BUI.Vars.CustomBar.Slash[i].enable then
			local icon=tostring(BUI.Vars.CustomBar.Slash[i].icon)
			table.insert(BUI.CustomBar.BarContent,{
...
function BUI.CustomBar.CustomBarUpdate(theme,slots,parent)
	local theme_color=BUI.Vars.Theme==6 and {1,204/255,248/255,1} or BUI.Vars.Theme==7 and BUI.Vars.AdvancedThemeColor or BUI.Vars.Theme>3 and BUI.Vars.CustomEdgeColor or {1,1,1,1}
	if theme then
		for i=1,20 do
	...
	UpdateContent()
	slots=slots or #BUI.CustomBar.BarContent
	local half		=slots>5 or (slots%2==0 and slots>2)
	local space		=2
	local h		= BUI.CustomBar.UltimateSlot.slot:GetHeight()*(half and .5 or 1)-(half and space/2 or 0)
	...
	if BUI.Vars.CustomBar.Enable then
		for i=1,slots do
			local leader=BUI.CustomBar.BarContent[i].leader
			local icon=BUI.CustomBar.BarContent[i].icon
			local command=leader and BUI.CustomBar.LeaderCommands[BUI.CustomBar.BarContent[i].command].mess or BUI.CustomBar.BarContent[i].command
...
--			ui[i].status	=BUI.UI.Texture("BUI_CustomSlot"..i.."Status", ui[i], {h-space,h-space}, {TOPLEFT,TOPLEFT,space/2,space/2}, "/EsoUI/Art/ActionBar/ActionSlot_toggledon.dds", true, 2)
			...
			ui[i].text		=BUI.UI.Label("BUI_CustomSlot"..i.."Text", ui[i], {h-space*2,h-space*2}, {TOPLEFT,TOPLEFT,space,space}, BUI.UI.Font("esobold",h*.4,true), nil, {1,1}, BUI.CustomBar.BarContent[i].text)
...
function BUI.CustomBar.UseCustomSlot(i)
	if BUI.CustomBar.BarContent[i].leader then
		--*|*PingMap(MAP_PIN_TYPE_PING,MAP_TYPE_LOCATION_CENTERED,.765+BUI.CustomBar.BarContent[i].command/100,.75)
	else
		BUI.CallLater("DoCommand",200,DoCommand,BUI.CustomBar.BarContent[i].command)
	end
end

function BUI.CustomBar.Initialize()
	Menu_Init()
	BUI.CustomBarUpdate()
end
...
-- Compatibility Exports, Should recaftor out of the calling code
BUI.CustomBar_Init = BUI.CustomBar.Initialize
BUI.UseCustomSlot = BUI.CustomBar.UseCustomSlot
BUI.CustomBarUpdate = BUI.CustomBar.CustomBarUpdate